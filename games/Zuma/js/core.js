class Marble {
    constructor({ color = `#ff2244` }) {
        this.ID = `${(~~(Math.random() * 1000000000))
            .toString(16)
            .toLocaleUpperCase()}`;
        this.DOM = createDiv(["marble", 'c-' + color.substring(1)]);
        this.Color = color;
...
class MarbleSprite {
	...
	render(marble, ctx) {
		var dx = marble.x - marble.px;
		var dy = marble.y - marble.py;
		var angle = Math.PI/2;
		...
		if(dx == 0 && dy == 0)
			...
			angle += Math.PI - Math.atan(dy/Math.abs(dx));
		else if (dx < 0 && dy < 0)
			angle += Math.atan(Math.abs(dy)/Math.abs(dx)) + Math.PI;
		else if (dx >= 0 && dy < 0)
			angle += 2*Math.PI - Math.atan(Math.abs(dy)/dx);
		marble.angle = angle;
		
		ctx.translate(marble.x * window.devicePixelRatio, marble.y * window.devicePixelRatio);
		ctx.rotate(angle);
		ctx.drawImage(this.image, 
						marble.sx, 
						marble.frame * this.height, 
						this.width, 
						this.height, 
						(-Marble.Size/2) * window.devicePixelRatio, 
						(-Marble.Size/2) * window.devicePixelRatio, 
						Marble.Size * window.devicePixelRatio, 
						Marble.Size * window.devicePixelRatio);
		ctx.setTransform(1, 0, 0, 1, 0, 0);
		if(dx != 0 || dy != 0) {
			marble.frame--;
			if(marble.frame < 0)
				marble.frame = marble.totalframes;
		}
	}
}
...
/*export*/ class Zuma {
    constructor(data) {
		...
		this.sprite = new MarbleSprite('./image/gameobjects.png');
    }
    get isInit() {
        return this._isInit;
    }
    ...
    attack() {
		...
		let audio = document.getElementById("fireballaudio");
		audio.loop = false;
		audio.pause();
		audio.load();
		audio.play();
    ...
    removeMarbleFromDataList(marble, index = this.marbleDataList.findIndex(d => d.marble.ID === marble.ID)) {
		...
		let audio = document.getElementById("chainaudio");
		audio.loop = false;
		audio.pause();
		audio.load();
		audio.play();
    ...
    drawCanvas() {
        ...
        this.marbleDataList.forEach(marble => {
			this.sprite.render(marble.marble, ctx);
        });
